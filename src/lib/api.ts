...
class ApiClient {
  ...
  private async request<T>(
    ...
    try {
      ...
      if (!data || Object.keys(data).length === 0) {
        ...
        return { success: true, data: {} as Property };
      }
  ...
  // Add: Upload multiple images (multipart/form-data)
  ...
  > {
    ...
    try {
      ...
      return {
        success: true,
        data: {
          images: payload?.images || payload?.files || [],
          urls: payload?.urls || [],
        },
  ...
  async createProperty(
    propertyData: Partial<Property> & { images?: string[] }
  ): Promise<ApiResponse<{ property?: Property }>> {
    return this.request<{ property?: Property }>("/properties", {
      method: "POST",
  ...
  async withdrawFromWallet(data: {
    amount: number;
    withdrawalMethod: "MTN_MOMO" | "ORANGE_MONEY";
    phone?: string;
    bankDetails?: {
      accountNumber: string;
      bankName: string;
