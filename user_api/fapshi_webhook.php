...
switch($event_type) {
    case 'charge.success':
        handleSuccessfulPayment($charge_data);
        break;
    case 'charge.failed':
        handleFailedPayment($charge_data);
        break;
    case 'charge.cancelled':
        handleCancelledPayment($charge_data);
        break;
    default:
...
function verifyFapshiSignature($payload, $signature) {
    ...
    $expected_signature = hash_hmac('sha256', $payload, $config['secret_key']);
    return hash_equals($expected_signature, $signature);
}

// Function to handle successful payment
function handleSuccessfulPayment($charge_data) {
    global $rstate;
    ...
    try {
        ...
        $update_transaction = $rstate->query("UPDATE tbl_wallet_transactions SET status = 'completed', fapshi_charge_id = '$charge_id', completed_at = NOW() WHERE transaction_id = '$transaction_id'");
...
function handleFailedPayment($charge_data) {
    global $rstate;
    ...
    $rstate->query("UPDATE tbl_wallet_transactions SET status = 'failed', fapshi_charge_id = '$charge_id', completed_at = NOW() WHERE transaction_id = '$transaction_id'");
    
    error_log("Payment failed for transaction: " . $transaction_id);
}

// Function to handle cancelled payment
function handleCancelledPayment($charge_data) {
    global $rstate;
    ...
    $rstate->query("UPDATE tbl_wallet_transactions SET status = 'cancelled', fapshi_charge_id = '$charge_id', completed_at = NOW() WHERE transaction_id = '$transaction_id'");
